...
class MyPacmanDQN(game.Agent):
    ...
    def getPolicy(self, state):
        if util.flipCoin(self.params['eps']):
            # Random:
            move = self.get_direction(random.randint(0, 4))
        else:
            ...
            if params['save_file']:
                if self.local_cnt > self.params['train_start'] and self.local_cnt % self.params['save_interval'] == 0:
                    torch.save(self.model.state_dict(),
                               'saves/model-' + params['save_file'] + "_" + str(self.cnt) + '_' + str(
                                   self.numeps) + '.pt')
                    print('Model saved')
